...
// Manual label offsets to avoid overlap in clustered European cities
// [dx, dy] relative to the dot position
...
export function WorldMap({
  ...
  lineColor = "#0ea5e9",
  showLabels = true,
  labelClassName: _labelClassName = "text-sm",
  animationDuration = 2,
  loop = true,
}: MapProps) {
            ...
            <g key={`points-group-${i}`}>
              <g key={`start-${i}`}>
                ...
                {showLabels && dot.start.label && (() => {
                  const [dx, dy] = labelOffsets[dot.start.label] || [0, -30];
                  return (
                    <motion.g
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center h-full">
                          <span className="text-[11px] font-medium px-1.5 py-0.5 rounded bg-black/90 text-white/90 border border-white/10 whitespace-nowrap">
                            {dot.start.label}
                          </span>
                        </div>
                      </foreignObject>
              ...
              <g key={`end-${i}`}>
                ...
                {showLabels && dot.end.label && (() => {
                  const [dx, dy] = labelOffsets[dot.end.label] || [0, -30];
                  return (
                    <motion.g
                      initial={{ opacity: 0, y: 5 }}
                      animate={{ opacity: 1, y: 0 }}
